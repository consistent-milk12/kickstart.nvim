  ...
  },
  {
    ...
    dependencies = {
      ...
      -- Mason must be loaded before its dependents so we need to set it up here.
      {
        ...
        config = function()
          ...
          -- Override vim.health.report to filter out unwanted language warnings
          local original_health_report = vim.health.report_warn or vim.health.warn
          if original_health_report then
            vim.health.report_warn = function(msg)
              -- Filter out warnings for languages we don't use
              if msg and (
                msg:match("Ruby:") or 
                msg:match("RubyGem:") or 
                msg:match("Composer:") or 
                msg:match("PHP:") or 
                msg:match("julia:")
              ) then
                return -- Suppress these warnings
              end
              return original_health_report(msg)
            end
            
            -- Also override the legacy function name
            vim.health.warn = vim.health.report_warn
          end
        end,
      },
    ...
    config = function()
        ...
        callback = function(event)
          ...
          local function client_supports_method(client, method, bufnr)
            if vim.fn.has 'nvim-0.11' == 1 then
              ...
              return client.server_capabilities and vim.tbl_get(client.server_capabilities, method) ~= nil or false
            end
          end
      ...
      vim.diagnostic.config {
        ...
        -- Disabled for tiny-inline-diagnostic.nvim
        virtual_text = false,
      }
      ...
      require('mason-lspconfig').setup {
        ...
        -- Only install servers we actually define
        ensure_installed_only = true,
        handlers = {
